  ...
  const [tasks,setTasks]=useState<task>({
    ...
    const hanlecreateList=():void=>{
        if(tasks.value.length!==0){
           axios.post("http://localhost:9000/create",tasks)
        }
        else{
    ...
    const handleValue=(e:React.ChangeEvent<HTMLInputElement>)=>{
      setTasks({...tasks,value:e.target.value})
    }
  return (
    <div className='flex pt-3 px-5 border border-transparent border-b-gray-500 shadow pb-3 justify-between'>
        ...
        <input value={tasks.value} onChange={handleValue} type={"text"} placeholder="Write a task" className=" bg-slate-100 pl-3 border border-blue-500 rounded-md w-[50%]" />
